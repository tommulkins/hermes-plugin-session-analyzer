...
function buildAskPrompt(s) {
  const lines = [];
  lines.push(
    "Analyze this Hermes agent session and tell me what went wrong and how to improve the next one.",
  );
  lines.push("");
  ...
  lines.push(
    `- Started: ${fmtTime(s.started_at)}  ·  Duration: ${fmtDur(s.duration_s)}`,
  );
  lines.push(
  ...
  lines.push("");
  lines.push(
    "Look up the full session transcript with the session_search tool:",
  );
  lines.push(
    `- Call session_search with session_id "${s.id}" to read it (large sessions return first/last bookends).`,
  );
  lines.push(
    "- To see the middle, scroll FORWARD: pass the LAST message id from the previous result as around_message_id. Message ids are global and NOT contiguous across sessions — never invent or guess an id, it errors.",
  );
  lines.push(
    "- Scroll windows overlap at the anchor by design; ignore duplicates and keep advancing from the newest last id.",
  );
  lines.push(
    "- Stop once you can explain every failed tool call listed above — do not fetch every remaining gap.",
  );
  lines.push("");
  ...
  host.notify(
    copied
      ? {
          ...
          message:
            "Prompt copied — paste it in (⌘V), pick your model, then send.",
          durationMs: 8000,
        }
      : {
          kind: "error",
          title: "Ask AI",
          message: "Clipboard unavailable — select and copy manually.",
        },
  );
...
function Stat({ label, value, title }) {
  return jsxs("div", {
    ...
    children: [
      jsx("div", {
        className:
          "text-[0.625rem] uppercase tracking-wide text-(--ui-text-quaternary)",
        children: label,
      }),
      jsx("div", {
...
function TokenBar({ input, output, cache }) {
  ...
  return jsxs("div", {
    ...
    children: [
      jsxs("div", {
        className:
          "flex h-2 w-full overflow-hidden rounded-full bg-(--ui-stroke-secondary)",
        children: [
          jsx("div", {
            style: { width: pct(input) + "%" },
            className: "h-full bg-(--ui-accent)",
            title: `input ${fmtTokens(input)}`,
          }),
          jsx("div", {
            style: { width: pct(cache) + "%" },
            className: "h-full bg-(--ui-accent) opacity-40",
            title: `cache read ${fmtTokens(cache)}`,
          }),
          jsx("div", {
            style: { width: pct(output) + "%" },
            className: "h-full bg-(--ui-text-tertiary)",
            title: `output ${fmtTokens(output)}`,
          }),
        ],
      }),
      jsxs("div", {
        ...
        children: [
          jsx("span", {
            children: `in ${fmtTokens(input)} (${pct(input).toFixed(0)}%)`,
          }),
          jsx("span", {
            className: "opacity-70",
            children: `cache ${fmtTokens(cache)} (${pct(cache).toFixed(0)}%)`,
          }),
          jsx("span", {
            children: `out ${fmtTokens(output)} (${pct(output).toFixed(0)}%)`,
          }),
        ],
      }),
    ],
        ...
        return jsxs("button", {
          ...
          children: [
            jsxs("div", {
              ...
              children: [
                jsx("span", {
                  ...
                  children:
                    s.title ||
                    (s.parent_session_id ? "Subagent" : s.id.slice(9)),
                }),
                showFailed && s.failed_count > 0
                  ? jsx(Badge, {
                      variant: "destructive",
                      className: "text-[0.6rem] shrink-0",
                      children: `${s.failed_count} failed`,
                    })
                  : null,
                s.pinned
                  ? jsx(Codicon, {
            ...
            searchMode
              ? jsx("div", {
                  className: cn(
                    "truncate text-[0.625rem] leading-snug",
                    active ? "opacity-80" : "text-(--ui-text-tertiary)",
                  ),
                  title: s.snippet
                    ? s.snippet.replace(/>>>|<<</g, "")
                    : undefined,
                  children: s.snippet
                    ? s.snippet.replace(/>>>/g, "“").replace(/<</g, "”")
                    : fmtTime(s.started_at),
                })
              : jsxs("div", {
                  className: cn(
                    "flex items-center gap-2 text-[0.625rem]",
                    active ? "opacity-80" : "text-(--ui-text-tertiary)",
                  ),
                  children: [
                    jsx("span", { children: fmtTime(s.started_at) }),
                    jsx("span", {
                      children: `${s.tool_call_count ?? 0} tools`,
                    }),
                    jsx("span", {
                      className: "tabular-nums",
                      children: fmtCost(s.estimated_cost_usd),
                    }),
                    s.parent_session_id && !s.title
                      ? jsx("span", {
                          className: "truncate font-mono",
                          title: `parent session: ${s.parent_session_id}`,
                          children: `parent: ${s.parent_session_id}`,
                        })
                      : null,
                  ],
                }),
            ...
            ? jsx("button", {
                ...
                onClick: () => {
                  const el = document.getElementById(
                    `failed-group-${name.replace(/[^a-zA-Z0-9_-]/g, "_")}`,
                  );
                  el?.scrollIntoView({ behavior: "smooth", block: "start" });
                },
                title: `Jump to failed ${name} calls`,
                className:
                  "flex items-center gap-1.5 min-w-0 font-mono text-xs truncate text-(--ui-error) hover:underline",
                children: name,
              })
            : jsx("span", {
      ...
      groups.map((gname) => {
        ...
        return jsxs(
          "div",
          {
            id: gid,
            className:
              "flex flex-col gap-0.5 rounded-md border border-(--ui-stroke-secondary) p-1",
            children: [
              jsxs("div", {
                ...
                children: [
                  jsx(Codicon, {
                    name: "error",
                    size: "0.75rem",
                    className: "shrink-0 text-(--ui-error)",
                  }),
                  jsx("span", {
              ...
              g.map((c) => {
                ...
                return jsxs(
                  ...
                  {
                    className:
                      "flex flex-col gap-0.5 rounded-md px-2 py-1 cursor-pointer hover:bg-(--chrome-action-hover)",
                    onClick: () => toggle(c.id + c.timestamp),
                    children: [
                      jsxs("div", {
                        className:
                          "flex items-center gap-2 min-w-0 text-left pointer-events-none",
                        children: [
                          jsx(Codicon, {
                          ...
                          jsx("span", {
                            ...
                            children: c.id ? c.id.slice(0, 24) : "(no id)",
                          }),
                          jsx("span", {
                            className:
                              "ml-auto shrink-0 text-[0.6rem] text-(--ui-text-tertiary)",
                            children: fmtTime(c.timestamp),
                          }),
                        ],
                      }),
                      c.error
                      ...
                      isOpen && Object.keys(args).length > 0
                        ? jsx("div", {
                            className:
                              "pl-4 font-mono text-[0.625rem] text-(--ui-text-tertiary) break-words whitespace-pre-wrap",
                            children: JSON.stringify(args, null, 1).slice(
                              0,
                              800,
                            ),
                          })
...
function FileRow({ f }) {
  ...
  return jsxs("div", {
    ...
    children: [
      jsx(Codicon, {
        name: write ? "file-code" : "file",
        size: "0.8rem",
        className: write ? "text-(--ui-accent)" : "text-(--ui-text-tertiary)",
      }),
      jsx("span", {
...
function Detail({ session, onOpenSession }) {
  ...
  const tokens = [
    jsx(Stat, {
      key: "in",
      label: "input",
      value: `${fmtTokens(session.input_tokens)} (${pct(session.input_tokens)}%)`,
      title: `input tokens: ${session.input_tokens ?? "—"}`,
    }),
    jsx(Stat, {
      key: "cache",
      label: "cache read",
    ...
    jsx(Stat, {
      key: "cost",
      label: "spend",
      value: fmtCost(session.estimated_cost_usd),
      title: session.cost_status ? `status: ${session.cost_status}` : undefined,
    }),
    jsx(Stat, {
      key: "dur",
      label: "duration",
      value: fmtDur(session.duration_s),
    }),
    jsx(Stat, {
      key: "msgs",
      label: "msgs",
  ...
  return jsxs("div", {
    ...
    children: [
      jsxs("div", {
        ...
        children: [
          ...
          session.about
            ? jsx("div", {
                className:
                  "text-xs text-(--ui-text-secondary) break-words line-clamp-2",
                title: session.about,
      ...
      jsx(TokenBar, {
        ...
        cache: session.cache_read_tokens,
      }),
      jsxs("div", {
      ...
      jsxs("div", {
        ...
        children: [
          session.summary
            ? jsxs("div", {
                className:
                  "flex-1 rounded-md border border-(--ui-stroke-secondary) px-2 py-1.5 text-xs text-(--ui-text-secondary)",
                children: [
                  jsx("span", {
          ...
          jsx("button", {
            ...
            title:
              "Open a new session that analyzes this one: what failed, how to make the next session more compact, config suggestions",
            children: [
                ...
                return jsxs(
                  "button",
                  {
                    ...
                    onClick: clickable
                      ? (e) => {
                          e.stopPropagation();
                          onOpenSession(sa.child_session_id);
                        }
                      : undefined,
                    title: clickable
                      ? `Open subagent session ${sa.child_session_id}`
                      : "No linked session",
                    className: cn(
                    ...
                    children: [
                      jsxs("div", {
                        ...
                        children: [
                          jsx(Codicon, {
                            name: label === "completed" ? "check" : "error",
                            size: "0.8rem",
                            className:
                              label === "completed"
                                ? "text-(--ui-accent)"
                          ...
                          jsx("span", {
                            ...
                            children: sa.model || sa.delegation_id,
                          }),
                          jsx(Badge, {
                            variant,
                            className: "text-[0.6rem] shrink-0",
                          ...
                          sa.duration_s != null
                            ? jsx("span", {
                                className:
                                  "ml-auto shrink-0 text-[0.6rem] text-(--ui-text-tertiary)",
                                children: fmtDur(sa.duration_s),
                              })
                            : null,
                          sa.api_calls > 0
                            ? jsx("span", {
                                className:
                                  "shrink-0 text-[0.6rem] text-(--ui-text-tertiary)",
                                children: `${sa.api_calls} call${sa.api_calls !== 1 ? "s" : ""}`,
                              })
                            : null,
                          sa.tokens && sa.tokens.input > 0
                            ? jsx("span", {
                                className:
                                  "shrink-0 text-[0.6rem] text-(--ui-text-tertiary)",
                                children: `${fmtTokens(sa.tokens.input)} in`,
                              })
                            : null,
                        ],
                      }),
                      sa.summary
                        ? jsx("div", {
                            className:
                              "pl-5 text-[0.625rem] text-(--ui-text-secondary) line-clamp-3",
                            title: sa.summary,
                            children: sa.summary,
                          })
                        : null,
              ...
              session.child_sessions.map((cs) =>
                jsxs(
                  "button",
                  {
                    ...
                    className:
                      "flex w-full items-center justify-between gap-2 rounded-md px-2 py-1 text-left text-[0.625rem] text-(--ui-text-tertiary) transition-colors hover:bg-(--chrome-action-hover) hover:text-foreground",
                    children: [
                      jsx("span", {
                        className: "truncate",
                        children: cs.title || cs.id,
                      }),
                      jsxs("span", {
                        ...
                        children: [
                          ...
                          jsx("span", {
                            children: fmtCost(cs.estimated_cost_usd),
                          }),
                        ],
                      }),
                    ],
...
function ContextMenu() {
  ...
  const item = (label, value) =>
    jsx(
      "button",
      {
        ...
        className:
          "flex w-full items-center gap-2 rounded-md px-2 py-1 text-left text-xs text-(--ui-text-secondary) transition-colors hover:bg-(--chrome-action-hover) hover:text-foreground",
        children: [
          jsx(Codicon, {
            name: "copy",
            size: "0.75rem",
  ...
  return jsxs("div", {
    className:
      "fixed z-50 flex min-w-[200px] max-w-[340px] flex-col gap-0.5 rounded-md border border-(--ui-stroke-secondary) bg-(--ui-bg-elevated) p-1 shadow-lg",
    ...
    children: [
      ...
      menu.parentId
        ? item(`Copy parent id: ${menu.parentId}`, menu.parentId)
        : null,
      jsx("div", {
        className:
          "px-2 py-0.5 text-[0.55rem] uppercase tracking-wide text-(--ui-text-quaternary)",
        children: menu.sessionId,
      }),
    ],
  });
}
...
function Page() {
  ...
  const {
    ...
    queryFn: () =>
      rest(`/sessions?limit=${limit}&q=${encodeURIComponent(q)}&sort=${sort}`),
    staleTime: 30_000,
  ...
  // Sash hover state is React state, not group-hover: the packaged app's CSS
  // build only generates classes its own code uses, so plugin-only arbitrary
  // classes (w-[9px], group-hover:opacity-100) never exist in the shipped
  // stylesheet. The separator's width is inline for the same reason.
    ...
    children: [
      ...
      jsxs("div", {
        ...
        children: [
          jsx("div", {
            ...
            children: [
              jsx("div", {
                ...
                children: [
                  jsx(Codicon, {
                    name: contentMode ? "sparkle" : "search",
                    size: "0.75rem",
                    className: cn(
                      "shrink-0",
                      contentMode
                  ...
                  jsx("input", {
                    ...
                    className:
                      "min-w-0 flex-1 bg-transparent text-xs outline-none placeholder:text-(--ui-text-quaternary)",
                  }),
                  ...
                  q
                    ? jsx("button", {
                        ...
                        className:
                          "shrink-0 text-(--ui-text-tertiary) hover:text-foreground",
                        title: "Clear search",
              ...
              jsx("div", {
                className:
                  "flex items-center gap-1 border-b border-(--ui-stroke-secondary) px-2 py-1",
                children: [
                  jsx("span", {
                    className:
                      "text-[0.55rem] uppercase tracking-wide text-(--ui-text-quaternary)",
                    children: "Sort",
                  }),
                  jsx("button", {
                    type: "button",
              ...
              jsx("div", {
                ...
                children:
                  (searchLoading && searching) || (isLoading && !searching)
                    ? jsx("div", {
                    ...
                    : (searchError && searching) || (error && !searching)
                      ? jsx("div", {
                          className: "p-4 text-sm text-(--ui-error)",
                          children: `Failed: ${(error || searchError)?.message ?? error ?? searchError}`,
                        })
                      : jsx(SessionList, {
                          sessions: searching
                            ? searchResults.map((r) => ({
                          ...
                          showFailed: sort === "failed",
                        }),
              }),
              !searching && sessions.length > 0 && list?.total > sessions.length
                ? jsx("div", {
                    className: "border-t border-(--ui-stroke-secondary) p-1.5",
                    children: jsx("button", {
                      type: "button",
                      onClick: () => setLimit((n) => Math.min(500, n + 200)),
                      className:
                        "w-full rounded-md px-2 py-1 text-[0.625rem] font-medium text-(--ui-text-secondary) transition-colors hover:bg-(--chrome-action-hover) hover:text-foreground",
          ...
          // Draggable divider — persistent hairline + a 0.25rem hover
          // band that fades in via opacity (--ui-sash-hover-border).
          // Width is INLINE and hover is React state: the packaged app's
          // CSS build only generates classes its own code uses, so
          // plugin-only arbitrary classes (w-[9px], group-hover:opacity-100)
          // never exist there. A missing width class collapses the strip
          // to 0px; pointer-events-none on the band then leaves nothing
          // hit-testable — no hover, no cursor, no drag.
          jsx("div", {
            style: { width: 9 },
            className:
              "group relative z-20 shrink-0 cursor-col-resize bg-transparent",
            onPointerDown: startDrag,
            onPointerMove: onDragMove,
            onPointerUp: endDrag,
            onPointerCancel: endDrag,
            onPointerEnter: () => setSashHover(true),
            onPointerLeave: () => setSashHover(false),
            role: "separator",
            ...
            children: jsxs("div", {
              ...
              children: [
                // Persistent hairline.
                jsx("span", {
                  style: { opacity: sashHover ? 1 : 0.1 },
                  className:
                    "absolute bg-(--ui-stroke-secondary) transition-opacity duration-100 inset-y-0 left-1/2 w-px -translate-x-1/2",
                }),
                // Hover band — the native sash hover element:
                // --ui-sash-hover-border, 0.25rem, opacity swap.
                jsx("span", {
                  style: { opacity: sashHover ? 1 : 0 },
                  className:
                    "absolute bg-(--ui-sash-hover-border) transition-opacity duration-100 inset-y-0 left-1/2 w-1 -translate-x-1/2",
                }),
              ],
            }),
          }),
          jsx("div", {
            ...
            children: detail
              ? jsx(Detail, {
                  session: detail,
                  onOpenSession: (id) => $selected.set(id),
                })
              : jsx("div", {
                  className:
...
export default {
  ...
  description:
    "Per-session analytics: tool calls, token/cache usage, spend, files touched, search.",
  defaultEnabled: true,
  register(ctx) {
    rest = ctx.rest;
    ctx.register({
